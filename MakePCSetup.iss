...
#define MyAppName "统一浏览器-新版本测试"
#define MyAppVersion "1.0.0.0"
#define MyAppPublisher "公司名"
#define MyAppURL "unnet.com"
; #define MyCopyright "林新发小屋所有"
#define MyAppPkgDir ".\pkgs\v1"
#define MyAppExeName "统一浏览器-新版本测试.exe"

; 生成 目录setup.exe 所在文件夹
#define MySetupOutDir ".\output"
#define MySetupOutBaseFilename "统一浏览器-新版本测试_setup"
...
var 
  ...
  imgBigIcon1, imgLogo1, imgOneKeySh, imgMolan:Longint;
...
begin
  w:=Round(577*pr/100);
  ImgSetPosition(imgProgressBar, DpiScale(38), DpiScale(350), DpiScale(w),DpiScale(29));
...
begin
  ...
  ImgSetPosition(imgProgressBarBg, DpiScale(38), DpiScale(350), DpiScale(577),DpiScale(19));
...
begin
  ...
  ImgSetVisibility(imgMolan,     isWpWelcome  or isWpFinished);
  TconSetVisible(lblAgree,          isWpWelcome);
  TconSetVisible(lblWelcome,        isWpWelcome or isWpFinished);
...
begin
  ...
  ExtractTemporaryFile('molan.png');
  ...
  imgMolan := ImgLoad(WizardForm.Handle,ExpandConstant('{tmp}\molan.png'),(160),(95),120,120,True,True);
  ImgSetVisibility(imgMolan,false);
  (*
  for i := 1 to InsBgAni_ImgCount do
  begin
...
var
  ...
  ResultCode: Integer;
begin
  if RegQueryStringValue(HKLM, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\123456789_is1', 'UninstallString', ResultStr) then
    begin
      ResultStr := RemoveQuotes(ResultStr);
      Exec(ResultStr, '/verysilent', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
    end;
    result := true;
  if RegQueryStringValue(HKLM, 'SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\123456789_is1', 'UninstallString', ResultStr) then
    begin
      ResultStr := RemoveQuotes(ResultStr);
      Exec(ResultStr, '/verysilent', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
    end;
